...
itemLinks.update = function() {
  ...
  for (const el of document.querySelectorAll(".object-link")) {
		const obj = w[el.dataset.objname.replace('@@@REPLACE_SPACE@@@', ' ')]
...
itemLinks.getVerbsLinks = function(obj){
  ...
  for (const o of verbArr) {
    ...
    s += `data-linkverb="${o}" data-objname="${obj.name.replace(' ','@@@REPLACE_SPACE@@@')}" data-objalias="${obj.alias}">${sentenceCase(o)}</span>`
	}
	return s
}
...
settings.nameTransformer = function(alias, obj, options = {}) {
	...
	let s = `<span class="object-link dropdown" data-objname="${obj.name.replace(' ', '@@@REPLACE_SPACE@@@')}"` 
  ...
  s += `obj="${obj.name}" class="droplink" name="${obj.name}-link">${alias}</span>`
	s += `<span obj="${obj.name.replace(' ', '@@@REPLACE_SPACE@@@')}" class="dropdown-content">`
	s += `<span obj="${obj.name.replace(' ', '@@@REPLACE_SPACE@@@')}-verbs-list-holder">`
